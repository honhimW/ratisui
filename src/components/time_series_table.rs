...
impl TimeSeriesValue {
    pub fn new(data: Vec<(u64, f64)>, offset: usize) -> Self {
        let mut vec = vec![];
        for (idx, (timestamp, value)) in data.iter().enumerate() {
            let data = Data {
    ...
    fn render_table(&mut self, frame: &mut Frame, area: Rect) {
        ...
        let rows = self.items.iter().enumerate().map(|(i, data)| {
            ...
            let item = [&data.index, &data.timestamp, &data.value];
            let height = 3;
...
impl Renderable for TimeSeriesValue {
...
impl Listenable for TimeSeriesValue {
...
fn constraint_len_calculator(items: &[Data]) -> (u16, u16, u16) {
    ...
    (index_len as u16, timestamp_len as u16, value_len as u16)
}
